...
if(isset($_POST['save_profile'])){
    ...
    $required_fields = ['education_level', 'gender', 'state_id', 'district_id', 'institution_type', 'age', 'marks', 'family_income', 'category', 'minority_status'];
    foreach($required_fields as $field) {
        if(empty($_POST[$field])) {
            die("<p style='color:red;'>Error: All required fields must be filled. Please fill in the $field field.</p>");
        }
    }
...
<form method="post" action="profile.php">
    ...
    <div id="course_year_fields" style="display:<?= ($profile && strpos($profile['education_level'], 'Below 10th') === false && strpos($profile['education_level'], '10th Pass') === false && strpos($profile['education_level'], 'Undergraduate') === false && strpos($profile['education_level'], 'Postgraduate') === false && strpos($profile['education_level'], 'PhD') === false) ? 'block' : 'none' ?>;">
        <input type="text" id="course" name="course" placeholder="Course" value="<?= $profile['course'] ?? '' ?>"><br><br>
        <input type="text" id="current_year" name="current_year" placeholder="Current Year" value="<?= $profile['current_year'] ?? '' ?>"><br><br>
    </div>
